...
export function etherscanLink(address: string, type: 'address' | 'tx' = 'address'): string {
  return `https://sepolia.etherscan.io/${type}/${address}`
}

export function formatSignedEth(wei: bigint): { text: string; isPositive: boolean; isZero: boolean } {
  const eth = Number(wei) / 1e18
  const isZero = Math.abs(eth) < 0.0001
  if (isZero) return { text: '0 ETH', isPositive: true, isZero: true }
  const sign = eth > 0 ? '+' : ''
  return {
    text: `${sign}${eth.toFixed(4)} ETH`,
    isPositive: eth >= 0,
    isZero: false,
  }
}
