...
var isChainSelectorSupported = (chainSelectorName) => Object.keys(ClientCapability.SUPPORTED_CHAIN_SELECTORS).includes(chainSelectorName);
...
function consensusCommonPrefixAggregation() {
  return simpleConsensus(AggregationType.COMMON_PREFIX);
}
function consensusCommonSuffixAggregation() {
...
function rayToBps(rayRate) {
  return rayRate * 10000n / RAY;
}
function compoundPerSecToBps(perSecRate) {
  return perSecRate * SECONDS_PER_YEAR * 10000n / WAD;
}
function readAaveRates(runtime2, protocolConfig) {
...
function readCompoundRates(runtime2, protocolConfig) {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: protocolConfig.chainSelectorName,
    isTestnet: false
  ...
  const utilResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: protocolConfig.contractAddress,
      data: utilCallData
  ...
  const supplyResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: protocolConfig.contractAddress,
      data: supplyCallData
  ...
  const borrowResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: protocolConfig.contractAddress,
      data: borrowCallData
...
function readMorphoRates(runtime2, protocolConfig) {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: protocolConfig.chainSelectorName,
    isTestnet: false
  ...
  if (!network248)
    throw new Error(`Network not found: ${protocolConfig.chainSelectorName}`);
  const evmClient = new cre.capabilities.EVMClient(network248.chainSelector.selector);
  const morphoCore = protocolConfig.contractAddress;
  const marketId = protocolConfig.assetAddress;
  const paramsCallData = encodeFunctionData({
    abi: MORPHO_BLUE_ABI,
    functionName: "idToMarketParams",
    args: [marketId]
  });
  const paramsResult = evmClient.callContract(runtime2, {
  ...
  const borrowRateResult = evmClient.callContract(runtime2, {
...
function readCTokenRates(runtime2, protocolConfig) {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName: protocolConfig.chainSelectorName,
    isTestnet: false
...
function readAllRates(runtime2, protocols) {
  const rates = [];
  for (const p of protocols) {
    if (!isChainSelectorSupported(p.chainSelectorName)) {
      runtime2.log(`  SKIP: ${p.protocol}/${p.chain} — chain ${p.chainSelectorName} not supported by CRE SDK`);
      continue;
    }
    runtime2.log(`  Reading ${p.protocol} on ${p.chain} [${p.asset}]...`);
    try {
      if (p.rateType === "aave" || p.rateType === "spark") {
        const rate = readAaveRates(runtime2, p);
        rates.push(rate);
        runtime2.log(`    supply=${rate.supplyBps}bps, borrow=${rate.borrowBps}bps`);
      } else if (p.rateType === "compound") {
        const rate = readCompoundRates(runtime2, p);
        rates.push(rate);
        runtime2.log(`    supply=${rate.supplyBps}bps, borrow=${rate.borrowBps}bps`);
      } else if (p.rateType === "morpho") {
        const rate = readMorphoRates(runtime2, p);
        rates.push(rate);
        runtime2.log(`    supply=${rate.supplyBps}bps, borrow=${rate.borrowBps}bps`);
      } else if (p.rateType === "ctoken") {
        const rate = readCTokenRates(runtime2, p);
        rates.push(rate);
        runtime2.log(`    supply=${rate.supplyBps}bps, borrow=${rate.borrowBps}bps`);
      }
    } catch (e) {
      runtime2.log(`    DROPPED ${p.protocol}/${p.chain}: ${e}`);
    }
  }
  const dropped = protocols.length - rates.length;
  if (dropped > 0) {
    runtime2.log(`  Collected ${rates.length}/${protocols.length} sources (${dropped} dropped — likely hit 15-call EVM limit)`);
  } else {
    runtime2.log(`  Collected ${rates.length}/${protocols.length} sources (all succeeded)`);
  }
  return rates;
}
function readChainlinkPrice(runtime2, feedName) {
  ...
  try {
    ...
    runtime2.log(`    Warning: ${feedName} price feed failed: ${e}`);
    return 0n;
  }
}
function readAllPrices(runtime2) {
  runtime2.log("  Reading Chainlink price feeds...");
  const ethUsd = readChainlinkPrice(runtime2, "ETH/USD");
  runtime2.log(`    ETH/USD: $${Number(ethUsd) / 1e8}`);
  const btcUsd = readChainlinkPrice(runtime2, "BTC/USD");
  runtime2.log(`    BTC/USD: $${Number(btcUsd) / 1e8}`);
  const usdcUsd = readChainlinkPrice(runtime2, "USDC/USD");
  runtime2.log(`    USDC/USD: $${Number(usdcUsd) / 1e8}`);
  return {
    ethUsd,
    btcUsd,
    usdcUsd,
...
function fetchSingleTVL(sendRequester, tvlApiBase, slug) {
...
function fetchAllTVLs(runtime2, protocols) {
  ...
  for (const p of protocols) {
    ...
    if (!slug)
      continue;
    runtime2.log(`  Fetching TVL for ${protocolBase} (${slug})...`);
    try {
      const tvlResult = httpClient.sendRequest(runtime2, fetchSingleTVL, ConsensusAggregationByFields({
        slug: () => identical(),
        tvl: () => median(),
        fetchedAt: () => ignore()
      ...
      if (tvlResult.tvl <= 0) {
        runtime2.log(`    ${protocolBase}: TVL unavailable — excluded from weighted average`);
      } else {
        runtime2.log(`    ${protocolBase}: TVL=$${Math.floor(tvlResult.tvl).toLocaleString()}`);
      }
    } catch (e) {
      runtime2.log(`    FAILED to fetch TVL for ${protocolBase}: ${e} — excluded from weighted average`);
      weights.push({
        protocol: protocolBase,
        tvlUsd: 0n
      });
  ...
  runtime2.log(`  Fetched TVL for ${weights.length} protocols`);
  return weights;
}
function mergeRatesWithTVL(rates, tvls) {
...
function computeRateDeviation(newRate, currentRate) {
  if (currentRate === 0n)
    return { deviationBps: 0n, safe: true };
  const newVal = new UInt64(newRate);
  const curVal = new UInt64(currentRate);
  const diff = newRate > currentRate ? newVal.sub(curVal).value : curVal.sub(newVal).value;
  return {
    deviationBps: diff,
    safe: diff <= 500n
  };
...
function computeBenchmark(weightedRates, previousRates, sourcesConfigured = 0n) {
...
function readSwapView(runtime2, functionName) {
  const evmClient = getSepoliaEvmClient(runtime2);
  const swapAddress = runtime2.config.swapAddress;
  const callData = encodeFunctionData({
    abi: DEBOR_SWAP_ABI,
    functionName,
  ...
  try {
    ...
    runtime2.log(`  Warning: ${functionName} call failed: ${e}`);
    return [];
  }
}
function readOracleRate(runtime2) {
  const evmClient = getSepoliaEvmClient(runtime2);
  const oracleAddress = runtime2.config.oracleAddresses.USDC;
  const callData = encodeFunctionData({
  ...
  try {
    ...
    runtime2.log(`  Warning: getRate call failed: ${e}`);
    return 0n;
  }
}
function readHistoricalRate(runtime2, periodsBack) {
  const evmClient = getSepoliaEvmClient(runtime2);
  const oracleAddress = runtime2.config.oracleAddresses.USDC;
  const callData = encodeFunctionData({
    abi: DEBOR_ORACLE_READ_ABI,
...
function writeSwapAction(runtime2, action, swapIds) {
  const swapAddress = runtime2.config.swapAddress;
  const reportData = encodeAbiParameters(parseAbiParameters("uint8, uint256[]"), [action, swapIds]);
  ...
  if (writeResult.txStatus !== TxStatus.SUCCESS) {
    runtime2.log(`  Write failed: ${writeResult.errorMessage || writeResult.txStatus}`);
    return "WRITE FAILED";
  }
  const txHash = writeResult.txHash || new Uint8Array(32);
  const txHashHex = bytesToHex(txHash);
  try {
    const receiptResult = evmClient.getTransactionReceipt(runtime2, {
      hash: hexToBase64(txHashHex)
    }).result();
    if (receiptResult.receipt) {
      const receipt = receiptResult.receipt;
      const receiptBlock = receipt.blockNumber ? protoBigIntToBigint(receipt.blockNumber) : 0n;
      runtime2.log(`  Receipt verified: status=${receipt.status}, gasUsed=${receipt.gasUsed}, block=${receiptBlock}, logs=${receipt.logs?.length || 0}`);
    }
  } catch (e) {
    runtime2.log(`  Receipt verification skipped: ${e}`);
  }
  return `TX: ${txHashHex}`;
}
function analyzeRateTrend(runtime2) {
  const evmClient = getSepoliaEvmClient(runtime2);
  const oracleAddress = runtime2.config.oracleAddresses.USDC;
  const noData = { direction: "INSUFFICIENT_DATA", velocity: 0n, acceleration: 0n };
  try {
    ...
    if (!logs.logs || logs.logs.length < 2)
      return noData;
    const recentRates = [];
    ...
    let upCount = 0;
    let downCount = 0;
    for (let i2 = 1;i2 < recentRates.length; i2++) {
      if (recentRates[i2] > recentRates[i2 - 1])
        upCount++;
      else if (recentRates[i2] < recentRates[i2 - 1])
        downCount++;
    }
    ...
    runtime2.log(`  Warning: filterLogs trend analysis failed: ${e}`);
    return { direction: "UNKNOWN", velocity: 0n, acceleration: 0n };
  }
}
var onBenchmarkUpdated = (runtime2, log) => {
  runtime2.log("=== CRE Anomaly Detector (EVM Log Trigger) ===");
  const blockNum = log.blockNumber ? String(log.blockNumber) : "unknown";
  runtime2.log(`  Event detected in block ${blockNum}`);
  if (log.txHash) {
    try {
      const evmClient = getSepoliaEvmClient(runtime2);
      const txHashHex = bytesToHex(log.txHash);
      const txResult = evmClient.getTransactionByHash(runtime2, {
        hash: hexToBase64(txHashHex)
      }).result();
      if (txResult.transaction) {
        const tx = txResult.transaction;
        const sender = bytesToHex(tx.to);
        runtime2.log(`  TX context: hash=${txHashHex}, to=${sender}, gas=${tx.gas}, nonce=${tx.nonce}`);
      }
    } catch (e) {
      runtime2.log(`  getTransactionByHash skipped: ${e}`);
    }
  }
  if (!runtime2.config.swapAddress) {
    runtime2.log("No swap address configured, skipping");
  ...
  try {
    ...
    runtime2.log(`  New rate from event: ${newRate} bps (${numSources} sources)`);
  } catch (e) {
    runtime2.log(`  Could not decode event data, reading oracle directly...`);
    newRate = readOracleRate(runtime2);
    runtime2.log(`  Current oracle rate: ${newRate} bps`);
  }
  const previousRate = readHistoricalRate(runtime2, 1n);
  if (previousRate === 0n) {
    runtime2.log("  No previous rate for comparison");
  ...
  if (settleableIds.length > 0) {
    runtime2.log("  Executing emergency settlement...");
  ...
  runtime2.log(`=== Anomaly Detector: ${summary} ===`);
  return summary;
};
var onSwapLifecycle = (runtime2, payload) => {
  runtime2.log("=== CRE Swap Lifecycle Manager ===");
  if (!runtime2.config.swapAddress) {
    runtime2.log("No swap address configured, skipping");
    return "SKIP: no swap address";
  }
  const results = [];
  runtime2.log("Phase 1: Rate spike detection...");
  const currentRate = readOracleRate(runtime2);
  const historicalRate = readHistoricalRate(runtime2, 2n);
  let spikeDetected = false;
  if (historicalRate > 0n) {
    const diff = currentRate > historicalRate ? currentRate - historicalRate : historicalRate - currentRate;
    ...
    runtime2.log("  No history for spike detection");
    results.push(`rate:${currentRate}bps`);
  }
  runtime2.log("Phase 2: Liquidation guard...");
  const atRiskIds = readSwapView(runtime2, "getAtRiskSwaps");
  runtime2.log(`  At-risk swaps: ${atRiskIds.length}`);
  if (atRiskIds.length > 0 || spikeDetected) {
    const emergencyIds = atRiskIds.length > 0 ? atRiskIds : readSwapView(runtime2, "getSettleableSwaps");
    if (emergencyIds.length > 0) {
      runtime2.log(`  Emergency settling ${emergencyIds.length} swaps...`);
      const tx = writeSwapAction(runtime2, 1, emergencyIds);
      runtime2.log(`  ${tx}`);
  ...
  if (expiredIds.length > 0) {
    runtime2.log(`  Batch closing ${expiredIds.length} expired swaps...`);
    const tx = writeSwapAction(runtime2, 2, expiredIds);
    runtime2.log(`  ${tx}`);
  }
  ...
  runtime2.log(`=== Swap Lifecycle Complete: ${summary} ===`);
  return summary;
};
function runPreflightCheck(runtime2) {
  const startTime = runtime2.now();
  runtime2.log("  [Preflight] Running health checks...");
  ...
  try {
    ...
    if (headerResult.header) {
      sepoliaBlock = headerResult.header.blockNumber ? protoBigIntToBigint(headerResult.header.blockNumber) : 0n;
      sepoliaTimestamp = headerResult.header.timestamp ? BigInt(headerResult.header.timestamp) : 0n;
    }
    runtime2.log(`    Sepolia block: ${sepoliaBlock}, timestamp: ${sepoliaTimestamp}`);
  } catch (e) {
    runtime2.log(`    [Preflight] headerByNumber failed: ${e}`);
  ...
  if (staleness > 300n) {
    runtime2.log(`    [Preflight] WARNING: Chain stale by ${staleness}s`);
  }
  const forwarderAddress = "0x15fC6ae953E024d975e77382eEeC56A9101f9F88";
  let forwarderBalance = 0n;
  try {
    const balanceResult = evmClient.balanceAt(runtime2, {
    ...
    const ethBalance = Number(forwarderBalance) / 1000000000000000000;
    runtime2.log(`    Forwarder balance: ${ethBalance.toFixed(6)} ETH`);
    if (forwarderBalance < 10000000000000000n) {
      runtime2.log(`    [Preflight] WARNING: Forwarder balance low!`);
    }
  } catch (e) {
    runtime2.log(`    [Preflight] balanceAt failed: ${e}`);
  }
  let swapBalance = 0n;
  if (runtime2.config.swapAddress) {
    try {
      const swapBalanceResult = evmClient.balanceAt(runtime2, {
        account: hexToBase64(runtime2.config.swapAddress),
        blockNumber: LATEST_BLOCK_NUMBER
      }).result();
      if (swapBalanceResult.balance) {
        swapBalance = protoBigIntToBigint(swapBalanceResult.balance);
      }
      runtime2.log(`    Swap contract balance: ${(Number(swapBalance) / 1000000000000000000).toFixed(6)} ETH`);
    } catch (e) {
      runtime2.log(`    [Preflight] swap balanceAt failed: ${e}`);
    }
  }
  let estimatedGas = 0n;
  const oracleAddress = runtime2.config.oracleAddresses.USDC;
  if (oracleAddress) {
    try {
      ...
      const gasResult = evmClient.estimateGas(runtime2, {
        msg: encodeCallMsg({
          from: zeroAddress,
          to: oracleAddress,
          data: dummyCallData
      ...
      runtime2.log(`    Estimated gas (getRate): ${estimatedGas}`);
    } catch (e) {
      runtime2.log(`    [Preflight] estimateGas failed: ${e}`);
    }
  }
  const endTime = runtime2.now();
  const executionMs = endTime.getTime() - startTime.getTime();
  runtime2.log(`    Execution time: ${executionMs}ms`);
  const healthy = sepoliaBlock > 0n;
  const message = healthy ? `OK: block=${sepoliaBlock}, fwd=${(Number(forwarderBalance) / 1000000000000000000).toFixed(4)}ETH, swap=${(Number(swapBalance) / 1000000000000000000).toFixed(4)}ETH, gas=${estimatedGas}, ${executionMs}ms` : "UNHEALTHY: could not read block";
  runtime2.log(`  [Preflight] ${message}`);
  return { sepoliaBlock, sepoliaTimestamp, forwarderBalance, swapBalance, estimatedGas, executionMs, healthy, message };
}
var PROTOCOL_SLUGS2 = new Map([
  ["aave_v3", "aave-v3"],
  ["compound_v3", "compound-v3"],
  ["spark", "spark"],
  ["morpho_blue", "morpho"]
]);
function fetchConfidentialTVL(runtime2, protocolSlug) {
  const confidentialClient = new cre.capabilities.ConfidentialHTTPClient;
  const apiBase = runtime2.config.tvlApiBase;
  const url = `${apiBase}/tvl/${protocolSlug}`;
  runtime2.log(`  [Confidential] Fetching TVL: ${protocolSlug}...`);
  try {
    ...
    if (!ok(response)) {
      runtime2.log(`    [Confidential] ${protocolSlug} returned status ${response.statusCode}`);
      return null;
    }
    const bodyStr = text(response);
    const tvlValue = Number(bodyStr.trim());
    if (isNaN(tvlValue) || tvlValue <= 0) {
      runtime2.log(`    [Confidential] Invalid TVL value for ${protocolSlug}`);
      return null;
    }
    runtime2.log(`    [Confidential] ${protocolSlug}: $${Math.floor(tvlValue).toLocaleString()}`);
    return {
      protocol: protocolSlug,
      tvlUsd: BigInt(Math.floor(tvlValue))
    };
  } catch (e) {
    runtime2.log(`    [Confidential] Failed for ${protocolSlug}: ${e}`);
    return null;
  }
}
...
function fetchTVLCheck(sendRequester, tvlApiBase, slug) {
  const url = `${tvlApiBase}/tvl/${slug}`;
  const response = sendRequester.sendRequest({ url, method: "GET" }).result();
  if (!ok(response))
    return { slug, tvl: 0 };
...
function runHttpValidation(runtime2) {
  runtime2.log("=== DeBOR HTTP Validation Layer ===");
  const warnings = [];
  runtime2.log("Step 1: Reading on-chain DeBOR oracle rates...");
  const targetNetwork = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.targetChainSelectorName,
    isTestnet: true
  ...
  if (!targetNetwork) {
    runtime2.log("  Target network not found");
  ...
  for (const asset of ASSETS2) {
    ...
    try {
      ...
      oracleRates[asset] = BigInt(rate);
      runtime2.log(`  ${asset}: ${oracleRates[asset]}bps (${Number(oracleRates[asset]) / 100}%)`);
    } catch (e) {
  ...
  for (const asset of ASSETS2) {
    ...
    if (rate === 0n) {
      ...
      runtime2.log(`  WARNING: ${asset} rate is 0 (no data or stale oracle)`);
    } else if (rate < RATE_MIN_BPS) {
      warnings.push(`${asset}:LOW(${rate}bps)`);
      runtime2.log(`  WARNING: ${asset} rate ${rate}bps suspiciously low`);
    } else if (rate > RATE_MAX_BPS) {
      warnings.push(`${asset}:HIGH(${rate}bps)`);
      runtime2.log(`  WARNING: ${asset} rate ${rate}bps suspiciously high`);
    } else {
      runtime2.log(`  ${asset}: OK (${rate}bps within ${RATE_MIN_BPS}-${RATE_MAX_BPS} range)`);
    }
  }
  ...
  if (stableRates.length >= 2) {
    ...
    if (spread > STABLECOIN_SPREAD_THRESHOLD) {
      ...
      runtime2.log(`  WARNING: Stablecoin spread ${spread}bps exceeds ${STABLECOIN_SPREAD_THRESHOLD}bps (${stableRates.map((r) => `${r.asset}=${r.rate}`).join(", ")})`);
    } else {
      runtime2.log(`  Stablecoins consistent: spread=${spread}bps (${stableRates.map((r) => `${r.asset}=${r.rate}`).join(", ")})`);
    }
  }
  runtime2.log("Step 4: DeFiLlama TVL cross-check...");
  const httpClient = new cre.capabilities.HTTPClient;
  const tvlSlugs = ["aave-v3", "compound-v3", "spark"];
  for (const slug of tvlSlugs) {
    try {
      const tvlResult = httpClient.sendRequest(runtime2, fetchTVLCheck, ConsensusAggregationByFields({
        slug: () => identical(),
        tvl: () => median()
      }))(runtime2.config.tvlApiBase, slug).result();
      if (tvlResult.tvl <= 0) {
        warnings.push(`TVL:${slug}=0`);
        runtime2.log(`  WARNING: ${slug} TVL is ${tvlResult.tvl} (protocol may be down)`);
      } else if (tvlResult.tvl < 1e7) {
        warnings.push(`TVL:${slug}=LOW`);
        runtime2.log(`  WARNING: ${slug} TVL=$${Math.floor(tvlResult.tvl).toLocaleString()} (unusually low)`);
      } else {
        runtime2.log(`  ${slug}: TVL=$${Math.floor(tvlResult.tvl).toLocaleString()} OK`);
      }
    } catch (e) {
      runtime2.log(`  ${slug}: TVL fetch failed: ${e}`);
    }
  }
    ...
    try {
      ...
      if (prev > 0n) {
        ...
        if (changePct > 50) {
          ...
          runtime2.log(`  WARNING: ${asset} rate changed ${changePct.toFixed(1)}% (${prev}→${current}bps)`);
        } else {
          runtime2.log(`  ${asset}: stable (${changePct.toFixed(1)}% change, ${prev}→${current}bps)`);
        }
      }
    } catch {
      runtime2.log(`  ${asset}: no historical data available`);
    }
  }
  runtime2.log("Step 6: DON consensus validation (4 top-level + 5 field-level strategies)...");
  let consensusChecks = 0;
  try {
    const configHash = runtime2.runInNodeMode((nodeRuntime) => {
    ...
    runtime2.log(`  6a [identical]: config consensus OK — ${configHash}`);
    consensusChecks++;
  } catch (e) {
    runtime2.log(`  6a [identical]: ${e} (expected in simulation)`);
  }
  try {
    const medianCount = runtime2.runInNodeMode((nodeRuntime) => nodeRuntime.config.protocols.length, consensusMedianAggregation())().result();
    runtime2.log(`  6b [median]: protocol count consensus = ${medianCount}`);
    consensusChecks++;
  } catch (e) {
    runtime2.log(`  6b [median]: ${e} (expected in simulation)`);
  }
  try {
    const assetPrefix = runtime2.runInNodeMode((nodeRuntime) => Object.keys(nodeRuntime.config.oracleAddresses), consensusCommonPrefixAggregation())().result();
    runtime2.log(`  6c [commonPrefix]: agreed on ${assetPrefix.length} assets — [${assetPrefix.join(", ")}]`);
    consensusChecks++;
  } catch (e) {
    runtime2.log(`  6c [commonPrefix]: ${e} (expected in simulation)`);
  }
  try {
    const addrSuffix = runtime2.runInNodeMode((nodeRuntime) => Object.values(nodeRuntime.config.oracleAddresses), consensusCommonSuffixAggregation())().result();
    runtime2.log(`  6d [commonSuffix]: agreed on ${addrSuffix.length} oracle addresses`);
    consensusChecks++;
  } catch (e) {
    runtime2.log(`  6d [commonSuffix]: ${e} (expected in simulation)`);
  }
  try {
    const fieldResult = runtime2.runInNodeMode((nodeRuntime) => ({
      ...
      configFingerprint: () => identical(),
      protocolCount: () => median(),
      assetNames: () => commonPrefix(),
      oracleAddrs: () => commonSuffix(),
      nodeLocalTimestamp: () => ignore()
    }))().result();
    runtime2.log(`  6e [fields]: fingerprint=${fieldResult.configFingerprint}, count=${fieldResult.protocolCount}, ` + `assets=${fieldResult.assetNames.length}, oracles=${fieldResult.oracleAddrs.length}, ts=${fieldResult.nodeLocalTimestamp}`);
    consensusChecks++;
  } catch (e) {
    runtime2.log(`  6e [fields]: ${e} (expected in simulation)`);
  }
  runtime2.log(`  Consensus checks passed: ${consensusChecks}/5`);
  runtime2.log("Step 7: DON-signed report distribution (sendReport)...");
  try {
    const validationPayload = encodeAbiParameters(parseAbiParameters("uint256, uint256, uint256"), [
      BigInt(warnings.length),
      BigInt(consensusChecks),
      BigInt(Math.floor(Date.now() / 1000))
    ]);
    const report2 = runtime2.report(prepareReportRequest(validationPayload)).result();
    const httpClient2 = new cre.capabilities.HTTPClient;
    httpClient2.sendReport(runtime2, report2, (reportResponse) => ({
      url: `${runtime2.config.tvlApiBase.replace("/tvl", "")}/webhook/debor-validation`,
      method: "POST",
    ...
    runtime2.log(`  sendReport: validation report distributed to webhook`);
  } catch (e) {
...
var protocolSchema = exports_external.object({
  ...
  rateType: exports_external.enum(["aave", "compound", "spark", "morpho", "ctoken"]),
  asset: exports_external.enum(["USDC", "ETH", "BTC", "DAI", "USDT"])
});
var configSchema = exports_external.object({
  schedule: exports_external.string(),
  protocols: exports_external.array(protocolSchema),
  tvlApiBase: exports_external.string(),
  targetChainSelectorName: exports_external.string(),
  oracleAddresses: exports_external.object({
    USDC: exports_external.string(),
    ETH: exports_external.string(),
    BTC: exports_external.string(),
    DAI: exports_external.string(),
...
var MIN_SOURCES = 3;
function runAssetBenchmark(runtime2, payload, asset) {
  runtime2.log(`=== DeBOR ${asset} Benchmark Update Starting ===`);
  const assetProtocols = runtime2.config.protocols.filter((p) => p.asset === asset);
  const sourcesConfigured = assetProtocols.length;
  runtime2.log(`Step 1: Reading ${sourcesConfigured} ${asset} rate sources from MAINNET...`);
  const rates = readAllRates(runtime2, assetProtocols);
  if (rates.length === 0) {
    runtime2.log(`ERROR: No ${asset} rates collected. Aborting.`);
    return `${asset}: No rates (0/${sourcesConfigured} configured)`;
  }
  if (rates.length < MIN_SOURCES) {
    runtime2.log(`ABORT: Only ${rates.length}/${sourcesConfigured} sources (minimum ${MIN_SOURCES}). Skipping write.`);
    return `${asset}: INSUFFICIENT_SOURCES (${rates.length}/${MIN_SOURCES} minimum)`;
  }
  runtime2.log(`  Collected ${rates.length}/${sourcesConfigured} sources`);
  runtime2.log("Step 2: Fetching TVL weights from DeFiLlama...");
  const tvls = fetchAllTVLs(runtime2, assetProtocols);
  runtime2.log(`Step 3: Computing DeBOR-${asset} benchmark...`);
  const weightedRates = mergeRatesWithTVL(rates, tvls);
  const metrics = computeBenchmark(weightedRates, [], BigInt(sourcesConfigured));
  runtime2.log(`  Metrics: ${stringifyMetrics(metrics)}`);
  runtime2.log(`  DeBOR_RATE:   ${metrics.deborRate} bps (${Number(metrics.deborRate) / 100}%)`);
  runtime2.log(`  DeBOR_SUPPLY: ${metrics.deborSupply} bps (${Number(metrics.deborSupply) / 100}%)`);
  runtime2.log(`  DeBOR_SPREAD: ${metrics.deborSpread} bps`);
  runtime2.log(`  DeBOR_VOL:    ${metrics.deborVol}`);
  runtime2.log(`  DeBOR_TERM7D: ${metrics.deborTerm7d} bps`);
  runtime2.log(`  Sources:      ${metrics.numSources}/${metrics.sourcesConfigured} configured`);
  const oracleAddress = runtime2.config.oracleAddresses[asset];
  if (!oracleAddress) {
  ...
  if (targetNetwork) {
    try {
      ...
      const currentRateResult = guardClient.callContract(runtime2, {
        call: encodeCallMsg({
          from: zeroAddress,
          to: oracleAddress,
          data: encodeFunctionData({
  ...
  const report2 = runtime2.report(prepareReportRequest(reportData)).result();
  runtime2.log(`Step 5: Writing DeBOR-${asset} to Sepolia...`);
  ...
  const writeResult = targetEvmClient.writeReport(runtime2, {
...
var onUsdcTrigger = (runtime2, payload) => {
  return runAssetBenchmark(runtime2, payload, "USDC");
};
var onEthTrigger = (runtime2, payload) => {
  return runAssetBenchmark(runtime2, payload, "ETH");
};
var onBtcTrigger = (runtime2, payload) => {
  return runAssetBenchmark(runtime2, payload, "BTC");
};
var onDaiTrigger = (runtime2, payload) => {
  return runAssetBenchmark(runtime2, payload, "DAI");
};
var onUsdtTrigger = (runtime2, payload) => {
  return runAssetBenchmark(runtime2, payload, "USDT");
};
var USDC_EXT_PROTOCOLS = [
...
var onUsdcExtTrigger = (runtime2, payload) => {
  runtime2.log("=== DeBOR USDC Extended Sources (merge 4 remaining protocols) ===");
  const allUsdcProtocols = runtime2.config.protocols.filter((p) => p.asset === "USDC");
  const extProtocols = allUsdcProtocols.filter((p) => USDC_EXT_PROTOCOLS.some((ext) => ext.protocol === p.protocol && ext.chain === p.chain));
  runtime2.log(`Step 1: Reading ${extProtocols.length} extended USDC sources...`);
  const extRates = readAllRates(runtime2, extProtocols);
  if (extRates.length === 0) {
    runtime2.log("  No extended rates collected. Skipping merge.");
    return "USDC_EXT: No rates (0 ext sources)";
  }
  runtime2.log(`  Collected ${extRates.length}/${extProtocols.length} ext sources`);
  runtime2.log("Step 2: Reading current oracle benchmark (core handler data)...");
  const oracleAddress = runtime2.config.oracleAddresses.USDC;
  const targetNetwork = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.targetChainSelectorName,
  ...
  try {
    ...
    const benchmarkResult = sepoliaClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: oracleAddress,
        data: encodeFunctionData({
    ...
    runtime2.log(`  Core benchmark: rate=${coreRate}bps, supply=${coreSupply}bps, sources=${coreSources}`);
  } catch (e) {
    runtime2.log(`  Core benchmark read failed: ${e} — writing ext-only`);
  }
  runtime2.log("Step 3: Fetching TVL weights for ext sources...");
  const extTvls = fetchAllTVLs(runtime2, extProtocols);
  ...
  runtime2.log(`  Ext benchmark: rate=${extMetrics.deborRate}bps, supply=${extMetrics.deborSupply}bps, sources=${extMetrics.numSources}`);
  runtime2.log("Step 4: Merging core + ext benchmarks...");
  const totalSources = coreSources + extMetrics.numSources;
  const mergedRate = totalSources > 0n ? (coreRate * coreSources + extMetrics.deborRate * extMetrics.numSources) / totalSources : extMetrics.deborRate;
  const mergedSupply = totalSources > 0n ? (coreSupply * coreSources + extMetrics.deborSupply * extMetrics.numSources) / totalSources : extMetrics.deborSupply;
  ...
  const mergedTerm = totalSources > 0n ? (coreTerm * coreSources + extMetrics.deborTerm7d * extMetrics.numSources) / totalSources : extMetrics.deborTerm7d;
  runtime2.log(`  Merged: rate=${mergedRate}bps, supply=${mergedSupply}bps, spread=${mergedSpread}bps, vol=${mergedVol}, sources=${totalSources}/${allUsdcProtocols.length}`);
  runtime2.log("Step 5: Writing merged USDC benchmark to oracle...");
  const triggerTimestamp = parseTriggerTimestamp(payload);
  const reportData = encodeAbiParameters(parseAbiParameters("uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256"), [mergedRate, mergedSupply, mergedSpread, mergedVol, mergedTerm, triggerTimestamp, totalSources, BigInt(allUsdcProtocols.length)]);
  const report2 = runtime2.report(prepareReportRequest(reportData)).result();
  const writeNetwork = getNetwork({
    chainFamily: "evm",
    chainSelectorName: runtime2.config.targetChainSelectorName,
...
var onHttpTrigger = (runtime2, payload) => {
  ...
  try {
    ...
    runtime2.log(`  HTTP input: ${inputStr}`);
    ...
    runtime2.log("  No valid input, defaulting to ALL assets");
  }
  runtime2.log(`  Refreshing: ${asset}`);
  if (asset === "ALL") {
    const results = [];
    for (const a of ["USDC", "ETH", "BTC", "DAI", "USDT"]) {
      results.push(runAssetBenchmark(runtime2, payload, a));
    }
    const summary = results.join(" | ");
    runtime2.log(`=== On-Demand Refresh Complete: ${summary} ===`);
    return summary;
  }
  const result = runAssetBenchmark(runtime2, payload, asset);
  runtime2.log(`=== On-Demand Refresh Complete: ${result} ===`);
  return result;
};
var onPreflightCheck = (runtime2, payload) => {
  runtime2.log("=== DeBOR Pre-flight & Price Context ===");
  const preflight = runPreflightCheck(runtime2);
  if (!preflight.healthy) {
    runtime2.log(`  Pre-flight FAILED: ${preflight.message}`);
    return `PREFLIGHT_FAIL: ${preflight.message}`;
  }
  runtime2.log("Step 2: Reading Chainlink price feeds...");
  const prices = readAllPrices(runtime2);
  ...
  if (prices.usdcUsd > 0n) {
    const pegDeviation = prices.usdcUsd > 100000000n ? prices.usdcUsd - 100000000n : 100000000n - prices.usdcUsd;
    const deviationBps = pegDeviation * 10000n / 100000000n;
    if (deviationBps > 50n) {
      runtime2.log(`  WARNING: USDC de-peg detected! ${deviationBps}bps deviation`);
    }
  }
  let confidentialTvl = "skipped";
  try {
    const secret = runtime2.getSecret({ id: "PREMIUM_API_KEY", namespace: "workflow" }).result();
    if (secret.value) {
      runtime2.log(`  Secret PREMIUM_API_KEY: loaded (${secret.value.length} chars)`);
      runtime2.log("Step 4b: Fetching confidential TVL (TEE-based)...");
      const result = fetchConfidentialTVL(runtime2, "aave-v3");
      confidentialTvl = result ? `$${Number(result.tvlUsd).toLocaleString()}` : "failed";
      runtime2.log(`  Confidential TVL (Aave V3): ${confidentialTvl}`);
    } else {
      runtime2.log(`  Secret PREMIUM_API_KEY: not configured — skipping confidential fetch`);
    }
  } catch {
    runtime2.log(`  Secret PREMIUM_API_KEY: not available (expected in simulation) — skipping confidential fetch`);
  }
  let consensusTimestamp = 0;
  try {
    const getNodeTimestamp = runtime2.runInNodeMode((nodeRuntime) => {
    ...
    runtime2.log(`  DON consensus timestamp: ${consensusTimestamp} (${new Date(consensusTimestamp * 1000).toISOString()})`);
  } catch (e) {
    runtime2.log(`  runInNodeMode consensus timestamp failed: ${e} (expected in simulation)`);
  }
  ...
  runtime2.log(`=== Pre-flight Complete: ${summary} ===`);
  return summary;
};
var initWorkflow = (config) => {
  ...
  if (!sepoliaNetwork)
    throw new Error("Sepolia network not found for trigger setup");
  ...
  return [
    cre.handler(cronCapability.trigger({ schedule: "0 0,30 * * * *" }), onUsdcTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 10,40 * * * *" }), onEthTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 20,50 * * * *" }), onBtcTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 15,45 * * * *" }), onDaiTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 18,48 * * * *" }), onUsdtTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 5,35 * * * *" }), onSwapLifecycle),
    cre.handler(cronCapability.trigger({ schedule: "0 25,55 * * * *" }), onPreflightCheck),
    cre.handler(httpCapability.trigger({ authorizedKeys: [] }), onHttpTrigger),
    cre.handler(cronCapability.trigger({ schedule: "0 2,32 * * * *" }), onUsdcExtTrigger),
    cre.handler(sepoliaEvmClient.logTrigger({
